...
void ow_rmt_driver_init() {
  ...
  ESP_ERROR_CHECK(
      rmt_driver_install( _config_rx.channel, OW_RMT_RX_BUFFER_SIZE,
                          ESP_INTR_FLAG_LOWMED | ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_SHARED ));
}
...
uint16_t ow_rmt_reset( void ) {
  ...
  // обычное состояние шины -- подтянуто к питанию. Создаём импульс сброса нужной длинны
  rmt_item32_t reset_ticks = OW_TICK_SLOT_MS_LOW_HIGH( _RESET_DURATION, 0 );
  if ( rmt_write_items( OW_RMT_TX_CHANNEL, &reset_ticks, 1, true ) == ESP_OK ) {
    RingbufHandle_t * _rb = get_rx_buffer();
    uint32_t length;
    rmt_item32_t * _items = ( rmt_item32_t * ) xRingbufferReceive( _rb, &length, 1 / portTICK_PERIOD_MS);
    if ( _items ) {
